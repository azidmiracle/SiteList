...
var ul_Childrn = ""; //checked the parent which is the UL tag
...
/*-----------------------------------------------------
The following functions are for the inApp browser plugIn
----------------------------------------------------*/

//to enable the inappbrowser
document.addEventListener("deviceready", onDeviceReady, false);
function onDeviceReady() {
  //
  var openSite = {
    init: function () {
      ulLinks.addEventListener("click", openSite.onEditClick);
    },
    onEditClick: function (e) {
      ...
      //check if the class name is the name
        ...
        let target = "_blank";
        let options =
          "location = yes,closebuttoncaption=Home,closebuttoncolor=#7c7646,navigationbuttoncolor=#8f3d00,hardwareback=yes,lefttoright=yes,hideurlbar=yes";
        inAppBrowserRef = cordova.InAppBrowser.open(url, target, options); //when the link is clicked, open the url
  ...
  document.addEventListener("deviceready", openSite.init);
}
...
btnAdd.addEventListener("click", function () {
  ...
  if (wantToAddOrEdit == true) {
    if (siteName.length > 0 && siteURL.length > 0) {
      ...
      EditOrAddMethod(EditOrAdd, siteName, siteURL); //call this method EditOrAddMethod
    } else {
...
/*-----------------------------------------------------
EditOrAddMethod method will add or update the
...
-----------------------------------------------------*/
function EditOrAddMethod(add_edit, siteName, siteURL) {
  if (add_edit == "add") {
...
function getKey(e) {
  ...
  ul_Childrn = li_tag.parentNode.children;
...
/*---------------------------------------------------------
DOM MANIPULATION: ADD LI TAG, DELETE LI TAG, UPDATE LIG TAG
-----------------------------------------------------------*/
/*-------------------------------
...
//function to delete the li element in client-side to avoid reloading the page
function deleteLink(ParentId, parentNodeChldrn, childToDelete) {
  console.log(Array.from(parentNodeChldrn))//show the converted array
  Array.from(parentNodeChldrn).forEach((child) => {
...
function updateLink(parentNodeChldrn, childToUpdate) {
  Array.from(parentNodeChldrn).forEach((child) => {
    if (childToUpdate == child) {
      child.children[0].innerHTML = siteName;//overwrite the sitename
      child.children[1].innerHTML = siteURL;//overwrite the url
    }
  });
}
  ...
  db.onsuccess = function (e) {
    ...
    request.onsuccess = function (event) {
      ...
      if (resultLen == 0) {
        ...
        statusMessage.textContent = "No data record";
      }
    };
  };
}
...
function deleteData(keypath) {
  ...
  objectStoreRequest.onsuccess = function (event) {
    ...
    deleteLink(ulLinks, ul_Childrn, li_tag); //it will delete tbe li tag without reloading the page.
  };
}
